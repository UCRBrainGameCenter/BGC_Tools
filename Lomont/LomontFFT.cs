...
namespace Lomont
{
    ...
    public class LomontFFT
    {
        ...
        public void FFT(float[] data, bool forward)
        {
            ...
            {
                int istep = 2 * mmax;
                float theta = sign * UnityEngine.Mathf.PI / mmax;
                float wr = 1, wi = 0;
                float wpr = UnityEngine.Mathf.Cos(theta);
                float wpi = UnityEngine.Mathf.Sin(theta);
        ...
        public void RealFFT(float[] data, bool forward)
        {
            ...
            { // do packed FFT. This can be changed to FFT to save memory                                        
                ...
                if (A != 1)
                {
                    float scale = UnityEngine.Mathf.Pow(2.0f, (A - 1) / 2.0f);
            ...
            float theta = B * sign * 2 * UnityEngine.Mathf.PI / n;
            float wpr = UnityEngine.Mathf.Cos(theta);
            float wpi = UnityEngine.Mathf.Sin(theta);
            float wjr = wpr;
            float wji = wpi;
            ...
            {
                float temp = data[0]; // unpack the y0 and y_{N/2}, then invert FFT                                
                ...
                {
                    float scale = UnityEngine.Mathf.Pow(2.0f, -(A + 1) / 2.0f) * 2;
                    for (int i = 0; i < data.Length; ++i)
                    {
                        data[i] *= scale;
                    }
        ...
        void Scale(float[] data, int n, bool forward)
        {
            ...
            if ((forward) && (A != 1))
            {
                float scale = UnityEngine.Mathf.Pow(n, (A - 1) / 2.0f);
                for (int i = 0; i < data.Length; ++i)
                {
                    data[i] *= scale;
                }
        ...
        void Initialize(int size)
        {
            ...
            {
                int istep = 2 * mmax;
                float theta = UnityEngine.Mathf.PI / mmax;
                float wr = 1, wi = 0;
                float wpi = UnityEngine.Mathf.Sin(theta);
                // compute in a slightly slower yet more accurate manner                                         
                float wpr = UnityEngine.Mathf.Sin(theta / 2);
                wpr = -2 * wpr * wpr;
                for (int m = 0; m < istep; m += 2)
                {
                    cosTable[pos] = wr;
