            ...
            int sessionElementIndex = UseStoredIndex)
        {
            if (LoadProtocolSet(protocolSetName))
            {
                if (protocolDictionary.ContainsKey(protocolKey))
                {
                    ...
                    // PlayerData is the source of truth for users with stored state — callers
                    // that don't have a specific jump target should pass UseStoredIndex (the
                    // default) and let CheckSequenceStatus resume from PlayerData. Callers
                    // that DO want to jump (e.g. the EditSession dialogs) pass explicit
                    // indices, which overwrite the persisted values via the property setters.
